...
def match_water_near_intensity_pair(data_list, laser_on_water_idx, laser_off_water_idx, expand_negative_pool = False):
    ...
    for int_idx, each_int_sum in enumerate(cmp_long_norm_sum):
        ...
        # diff_each_pulse = cmp_long_norm_sum[int_idx] - cmp_short_norm_sum[sim_sum_idx]
        # print(str(int_idx) + " : " + str(diff_each_pulse))
    return neareast_int_pair


def plot_sum_for_criteria(data, graph_title, v_line_1=0.0, v_line_2=0.0, v_line_3=0.0, v_line_4=0.0, v_line_5=0.0):
    """
    ...
    if v_line_4 != 0.0:
        plt.axvline(x=v_line_4, color='y')
    if v_line_5 != 0.0:
        plt.axvline(x=v_line_5, color='y')
    # plt.xlim(0, 700000) # todo
    plt.show()
